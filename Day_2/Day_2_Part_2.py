...
for game in data.split('\n'):
    ...
    for results in game_dep.split(";"):
        for color in results.split(","):
            if "red" in color:
                try:
                    if int(f"{color[1]}{color[2]}") > red_max:
                        red_max = int(f"{color[1]}{color[2]}")
                except ValueError:
                    if int(color[1]) > red_max:
                        red_max = int(color[1])
            if "green" in color:
                try:
                    if int(f"{color[1]}{color[2]}") > green_max:
                        green_max = int(f"{color[1]}{color[2]}")
                except ValueError:
                    if int(color[1]) > green_max:
                        green_max = int(color[1])
            if "blue" in color:
                try:
                    if int(f"{color[1]}{color[2]}") > blue_max:
                        blue_max = int(f"{color[1]}{color[2]}")
                except ValueError:
                    if int(color[1]) > blue_max:
                        blue_max = int(color[1])
    game_puissance = red_max * green_max * blue_max
    result += game_puissance
print(result)
