...
dico_words = {"one": 1, "two": 2, "three": 3, "four": 4, "five": 5, "six": 6, "seven": 7, "eight": 8, "nine": 9}
...
for inp in inps.split('\n'):
  ...
  for i in range(len(inp)):
    ...
    try :
      founds.append(int(inp[i]))
    except ValueError:
      try:
        for y in range(len(words)):    
          if f'{inp[i]}{inp[i+1]}{inp[i+2]}' in words[y][0:3]:
            founds.append(dico_words[words[y]])
      except :
        pass
  founds = f"{founds[0]}{founds[-1]}"
